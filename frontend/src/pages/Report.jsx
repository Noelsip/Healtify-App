                ...
                {submitStatus === 'success' && (
                    <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4 flex items-start gap-3 animate-fade-in">
                        ...
                        <div className="flex-1">
                            <h4 className="font-semibold text-green-800 mb-1 text-sm sm:text-base">{t('report.success.title')}</h4>
                ...
                {(submitStatus === 'error' || errorMessage) && (
                    <div className="mb-6 bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3 animate-fade-in">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xs sm:text-sm text-red-600">{errorMessage || t('report.errors.submitFailed')}</p>
                        </div>
                    </div>
                )}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                        ...
                        <section className="space-y-3 sm:space-y-4">
                            <label htmlFor="claim_text" className="block text-base sm:text-lg font-semibold text-slate-700">
                                {t('report.claimLabel')} <span className="text-red-500">*</span>
                            </label>
                            <textarea
                                id="claim_text"
                                name="claim_text"
                                ...
                                placeholder={t('report.claimPlaceholder')}
                        ...
                        <section className="space-y-3 sm:space-y-4">
                            <h3 className="text-base sm:text-lg font-semibold text-slate-700">{t('report.reporterInfo')}</h3>
...
{t('report.email')}
                        ...
                        {/* Field {t('report.supportingEvidence')} */}
                        <section className="space-y-3 sm:space-y-4">
                            <label className="block text-base sm:text-lg font-semibold text-slate-700">
                                {t('report.supportingEvidence')} <span className="text-red-500">*</span>
                            </label>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 sm:gap-6 text-xs sm:text-sm font-medium text-slate-600">
                                <label className="flex items-center space-x-2 cursor-pointer">
                                    <input
                                        ...
                                        disabled={isSubmitting}
                                    />
                                    <span>{t('report.evidenceTypes.doi')}</span>
                                </label>
                                <label className="flex items-center space-x-2 cursor-pointer">
                                    <input
                                        type="radio"
                                        ...
                                        disabled={isSubmitting}
                                    />
                                    <span>{t('report.evidenceTypes.pdf')}</span>
                                </label>
                            </div>
                            ...
                            {buktiType === 'doi_link' && (
                                <div className="space-y-2 animate-fade-in">
                                    <label htmlFor="supporting_doi" className="block text-xs sm:text-sm font-poppins text-slate-600">
                                        {t('report.doiLabel')} <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="url"
                                        id="supporting_doi"
                                        ...
                                        onChange={handleInputChange}
                                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 transition duration-150 placeholder-gray-400 text-sm"
                                        placeholder={t('report.doiPlaceholder')}
                                        required={buktiType === 'doi_link'}
                                        disabled={isSubmitting}
                                    />
                                    <p className="text-xs text-slate-500">{t('report.doiHelper')}</p>
                                </div>
                            )}
                                ...
                                <div className="space-y-3 animate-fade-in">
                                    <div className="space-y-2">
                                        <label htmlFor="pdf_upload" className="block text-xs sm:text-sm font-poppins text-slate-600">
                                            {t('report.pdfLabel')} <span className="text-red-500">*</span>
                                        </label>
                                        ...
                                        <p className="text-xs text-slate-500">{t('report.pdfHelper')}</p>
                                    </div>
                        ...
                        <div className="bg-yellow-50 border-l-4 border-yellow-500 p-3 sm:p-4 rounded-lg text-xs sm:text-sm text-yellow-800">
                            <p className="font-semibold mb-1">{t('report.note.title')}</p>
                            <p>{t('report.note.content')}</p>
                        </div>
                    </form>
                </div>
            </main>
